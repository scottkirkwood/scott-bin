...
var (
	...
	quertyTypoMap = map[string]string{}
)

func init() {
	removeall := strings.NewReplacer(":", "", "[", "", "]", "")
	removesome := strings.NewReplacer(":", "", "]", "")
	generateQuertyTypos(quertyLower)
	generateQuertyTypos(quertyShifted)
	for _, cmd := range knownCommands {
		blacklist[removeall.Replace(cmd)] = true
		name := strings.Split(removesome.Replace(cmd), "[")
		if len(name) == 2 {
			blacklist[name[0]] = true
		}
	}
}

func generateQuertyTypos(table [][]string) {
	maxX := 0
	...
	m := quertyTypoMap // Alias
	maxY := len(table)
	for x := 0; x < maxX; x++ {
		for y := 0; y < maxY; y++ {
			ch := getXY(quertyLower, x, y)
			if ch != "" {
				m[ch] = getXY(quertyLower, x-1, y-1) +
					getXY(quertyLower, x, y-1) +
					getXY(quertyLower, x+1, y-1) +
					getXY(quertyLower, x-1, y) +
					getXY(quertyLower, x+1, y) +
					getXY(quertyLower, x-1, y+1) +
					getXY(quertyLower, x, y+1) +
					getXY(quertyLower, x+1, y+1)
...
func getXY(table [][]string, x, y int) string {
	if x < 0 || y < 0 || y >= len(table) {
		return ""
	}
	row := table[y]
	if x >= len(row) {
...
func swaps(input string) (ret []string) {
	for i := 0; i < len(input)-1; i++ {
		ret = append(ret, input[:i]+input[i+1:i+2]+input[i:i+1]+input[i+2:])
	}
	return ret
}

func capFirst(input string) (ret []string) {
	// Common typo after shift-colon is to capitalize the first character
	ret = append(ret, strings.ToUpper(input[:1])+input[1:])
	return ret
}

func deletes(input string) (ret []string) {
	// The first character isn't deleted as it is often typed correctly.
	for i := 1; i < len(input); i++ {
		ret = append(ret, input[:i]+input[i+1:])
	}
	return ret
}

func quertyTypos(input string) (ret []string) {
	for i := 0; i < len(input); i++ {
		chTypos := quertyTypoMap[input[i:i+1]]
		for j := 0; j < len(chTypos); j++ {
			ret = append(ret, input[:i]+chTypos[j:j+1]+input[i+1:])
		}
	}
	return ret
}

func All(input string) (ret []string) {
	ret = append(ret, swaps(input)...)
	ret = append(ret, capFirst(input)...)
	ret = append(ret, deletes(input)...)
	ret = append(ret, quertyTypos(input)...)
	return ret
}
...
func main() {
	...
	fmt.Printf("\" Command typos\n")
	for _, input := range cmds {
		fmt.Printf("\n\" Typos for %s\n", input)
		for _, edit := range All(input) {
			if !blacklist[edit] {
				fmt.Printf("cabbrev %s %s\n", edit, input)
			}
		}
	}
}
...
var (
	quertyLower = [][]string{ // my Advantage Kinesis
		{"=", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-"},
		{"", "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "\\"},
		{"", "a", "s", "d", "f", "g", "h", "j", "k", "l", ";", "'"},
		{"", "z", "x", "c", "v", "b", "n", "m", ",", ".", "/"},
